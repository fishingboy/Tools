...
function fileQueued(fileObj) {    
    ...
    progress.SetStatus("ready to uploading...");
...
function uploadComplete(fileObj, server_data) {
	try {
		...
		AddImage("images/ppt.gif", server_data);
...
function fileComplete(fileObj) {
	...
	try {
		...
		//} else {
			...
			progress.SetStatus("All data received.");
			progress.ToggleCancel(false);
...
function AddImage(src, id) {
	...
	new_img.onclick = function () {OpenPPT(id); };
	new_img.src = src;
}
